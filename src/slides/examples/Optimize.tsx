...
export default function ExamplesError() {
  ...
  return (
    <SlideFrame
      ...
      notes={
        <>
          Также трейс может подсветить плохой код.
          <br />
          Например, в этом трейсе мы видим, что в операции <i>GetStatus</i>{' '}
          делается несколько sql вызовов подряд.
          <br />
          Некоторые запросы отличаются только одним параметром
          <br />
          Чаще всего такой код является антипаттерном. Такой код может быть
          оптимизирован: как минимум, можно достать все данные одним запросом -
          тогда время выполнения сократится за счет уменьшения количества
          сетевых вызовов.
          <br />
