...
export default function AdvancedEnv() {
  return (
    <SlideFrame
      ...
      notes={
        <>
          Но этот код можно значительно сократить, так как большинство настроек
          можно вынести в переменные окружения.
          <p>
            В коде создается только TracerProvider, а все остальное управляется
            через переменные. Это удобно потому как не нужно перекомпилировать
            код, чтобы поменять конфиг.
          </p>
        </>
      }
    >
