...
export default function IntegrationKafka() {
  return (
    <SlideFrame
      ...
      notes={
        <>
          В случае, если сервисы общаются по HTTP или GRPC, всё работает «из
          коробки» — <i>traceID</i> прокидывается в заголовках автоматически.
          <p>
            Для остальных случаев нужно <b>вручную</b> выполнять пропагацию{' '}
            <i>trace context</i>.
          </p>
          <p>
            Например, для <b>Kafka</b> в <i>producer</i>, с помощью пакета{' '}
            <i>propagation</i>, нужно достать значения <i>trace context</i> и
            записать его в заголовки передаваемого сообщения.
          </p>
        </>
      }
    >
